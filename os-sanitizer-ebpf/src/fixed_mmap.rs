...
use os_sanitizer_common::{
    FixedMmapViolation, OsSanitizerError, OsSanitizerReport, ProgId, EXECUTABLE_LEN,
};
...
fn uprobe_fixed_mmap_safe_function(probe: ProbeContext) -> u32 {
    ...
    update_tracking(pid_tgid, ProgId::uprobe_fixed_mmap_safe_function);
...
fn uretprobe_fixed_mmap_safe_function(_probe: RetProbeContext) -> u32 {
    let pid_tgid = bpf_get_current_pid_tgid();
    update_tracking(pid_tgid, ProgId::uretprobe_fixed_mmap_safe_function);
...
unsafe fn try_fentry_fixed_mmap(probe: &FEntryContext) -> Result<u32, OsSanitizerError> {
    let pid_tgid = bpf_get_current_pid_tgid();
    update_tracking(pid_tgid, ProgId::fentry_fixed_mmap);
